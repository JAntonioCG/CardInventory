-- Rare holo
CALL AddCardToInventory(
    'Base Set',                	-- Expansión
    'Base Set',                 -- Subexpansión
    'Alakazam',        			-- Nombre
    'Rara Holo',                -- Rareza
    'Psychic',                	-- Categoría
    '001/102',                    -- Número
	'Good',                     -- Condición
	8.50,                      -- Precio
    12,                         -- Stock
    'https://images.pokemontcg.io/base1/1_hires.png'  -- URL
);
CALL AddCardToInventory(
    'Base Set',                	-- Expansión
    'Base Set',                 -- Subexpansión
    'Blastoise',        			-- Nombre
    'Rara Holo',                -- Rareza
    'Water',                	-- Categoría
    '002/102',                    -- Número
	'Good',                     -- Condición
	8.50,                      -- Precio
    12,                         -- Stock
    'https://images.pokemontcg.io/base1/2_hires.png'  -- URL
);
CALL AddCardToInventory(
    'Base Set',                	-- Expansión
    'Base Set',                 -- Subexpansión
    'Chansey',        			-- Nombre
    'Rara Holo',                -- Rareza
    'Colorless',                	-- Categoría
    '003/102',                    -- Número
	'Good',                     -- Condición
	8.50,                      -- Precio
    12,                         -- Stock
    'https://images.pokemontcg.io/base1/3_hires.png'  -- URL
);
CALL AddCardToInventory(
    'Base Set',                	-- Expansión
    'Base Set',                 -- Subexpansión
    'Charizard',        		-- Nombre
    'Rara Holo',                -- Rareza
    'Fire',                	-- Categoría
    '004/102',                    -- Número
	'Good',                     -- Condición
	8.50,                      -- Precio
    12,                         -- Stock
    'https://images.pokemontcg.io/base1/4_hires.png'  -- URL
);
CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Clefairy', 
    'Rare Holo', 
    'Colorless', 
    '005/102', 
    'Good', 
    6.50, 
    10, 
    'https://images.pokemontcg.io/base1/5_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Gyarados', 
    'Rare Holo', 
    'Water', 
    '006/102', 
    'Good', 
    7.00, 
    8, 
    'https://images.pokemontcg.io/base1/6_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Hitmonchan', 
    'Rare Holo', 
    'Fighting', 
    '007/102', 
    'Good', 
    5.00, 
    15, 
    'https://images.pokemontcg.io/base1/7_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Machamp', 
    'Rare Holo', 
    'Fighting', 
    '008/102', 
    'Good', 
    8.00, 
    10, 
    'https://images.pokemontcg.io/base1/8_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Magneton', 
    'Rare Holo', 
    'Lightning', 
    '009/102', 
    'Good', 
    9.00, 
    7, 
    'https://images.pokemontcg.io/base1/9_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Mewtwo', 
    'Rare Holo', 
    'Psychic', 
    '010/102', 
    'Good', 
    12.00, 
    6, 
    'https://images.pokemontcg.io/base1/10_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Nidoking', 
    'Rare Holo', 
    'Grass', 
    '011/102', 
    'Good', 
    7.50, 
    10, 
    'https://images.pokemontcg.io/base1/11_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Ninetales', 
    'Rare Holo', 
    'Fire', 
    '012/102', 
    'Good', 
    8.50, 
    12, 
    'https://images.pokemontcg.io/base1/12_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Poliwrath', 
    'Rare Holo', 
    'Water', 
    '013/102', 
    'Good', 
    9.50, 
    10, 
    'https://images.pokemontcg.io/base1/13_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Raichu', 
    'Rare Holo', 
    'Lightning', 
    '014/102', 
    'Good', 
    10.00, 
    8, 
    'https://images.pokemontcg.io/base1/14_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Venusaur', 
    'Rare Holo', 
    'Grass', 
    '015/102', 
    'Good', 
    15.00, 
    5, 
    'https://images.pokemontcg.io/base1/15_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Zapdos', 
    'Rare Holo', 
    'Lightning', 
    '016/102', 
    'Good', 
    14.00, 
    6, 
    'https://images.pokemontcg.io/base1/16_hires.png'
);
-- Rare
CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Beedrill', 
    'Rare', 
    'Grass', 
    '017/102', 
    'Good', 
    4.50, 
    10, 
    'https://images.pokemontcg.io/base1/17_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Dragonair', 
    'Rare', 
    'Colorless', 
    '018/102', 
    'Good', 
    5.50, 
    12, 
    'https://images.pokemontcg.io/base1/18_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Dugtrio', 
    'Rare', 
    'Fighting', 
    '019/102', 
    'Good', 
    4.00, 
    15, 
    'https://images.pokemontcg.io/base1/19_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Electabuzz', 
    'Rare', 
    'Lightning', 
    '020/102', 
    'Good', 
    3.50, 
    18, 
    'https://images.pokemontcg.io/base1/20_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Electrode', 
    'Rare', 
    'Lightning', 
    '021/102', 
    'Good', 
    5.00, 
    8, 
    'https://images.pokemontcg.io/base1/21_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Pidgeotto', 
    'Rare', 
    'Colorless', 
    '022/102', 
    'Good', 
    6.00, 
    10, 
    'https://images.pokemontcg.io/base1/22_hires.png'
);
-- Uncommon
CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Arcanine', 
    'Uncommon', 
    'Fire', 
    '023/102', 
    'Good', 
    3.00, 
    10, 
    'https://images.pokemontcg.io/base1/23_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Charmeleon', 
    'Uncommon', 
    'Fire', 
    '024/102', 
    'Good', 
    2.50, 
    12, 
    'https://images.pokemontcg.io/base1/24_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Dewgong', 
    'Uncommon', 
    'Water', 
    '025/102', 
    'Good', 
    2.00, 
    15, 
    'https://images.pokemontcg.io/base1/25_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Dratini', 
    'Uncommon', 
    'Dragon', 
    '026/102', 
    'Good', 
    1.50, 
    20, 
    'https://images.pokemontcg.io/base1/26_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Farfetch\'d', 
    'Uncommon', 
    'Colorless', 
    '027/102', 
    'Good', 
    2.00, 
    18, 
    'https://images.pokemontcg.io/base1/27_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Growlithe', 
    'Uncommon', 
    'Fire', 
    '028/102', 
    'Good', 
    2.00, 
    15, 
    'https://images.pokemontcg.io/base1/28_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Haunter', 
    'Uncommon', 
    'Ghost', 
    '029/102', 
    'Good', 
    3.50, 
    12, 
    'https://images.pokemontcg.io/base1/29_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Ivysaur', 
    'Uncommon', 
    'Grass', 
    '030/102', 
    'Good', 
    2.50, 
    14, 
    'https://images.pokemontcg.io/base1/30_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Jynx', 
    'Uncommon', 
    'Psychic', 
    '031/102', 
    'Good', 
    3.00, 
    10, 
    'https://images.pokemontcg.io/base1/31_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Kadabra', 
    'Uncommon', 
    'Psychic', 
    '032/102', 
    'Good', 
    3.00, 
    15, 
    'https://images.pokemontcg.io/base1/32_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Kakuna', 
    'Uncommon', 
    'Grass', 
    '033/102', 
    'Good', 
    1.50, 
    20, 
    'https://images.pokemontcg.io/base1/33_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Machoke', 
    'Uncommon', 
    'Fighting', 
    '034/102', 
    'Good', 
    2.00, 
    15, 
    'https://images.pokemontcg.io/base1/34_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Magikarp', 
    'Uncommon', 
    'Water', 
    '035/102', 
    'Good', 
    1.00, 
    25, 
    'https://images.pokemontcg.io/base1/35_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Magmar', 
    'Uncommon', 
    'Fire', 
    '036/102', 
    2.50, 
    'Good', 
    18, 
    'https://images.pokemontcg.io/base1/36_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Nidorino', 
    'Uncommon', 
    'Poison', 
    '037/102', 
    'Good', 
    2.00, 
    20, 
    'https://images.pokemontcg.io/base1/37_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Poliwhirl', 
    'Uncommon', 
    'Water', 
    '038/102', 
    'Good', 
    2.50, 
    15, 
    'https://images.pokemontcg.io/base1/38_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Porygon', 
    'Uncommon', 
    'Colorless', 
    '039/102', 
    'Good', 
    2.00, 
    18, 
    'https://images.pokemontcg.io/base1/39_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Raticate', 
    'Uncommon', 
    'Normal', 
    '040/102', 
    'Good', 
    1.50, 
    20, 
    'https://images.pokemontcg.io/base1/40_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Seel', 
    'Uncommon', 
    'Water', 
    '041/102', 
    'Good', 
    2.00, 
    15, 
    'https://images.pokemontcg.io/base1/41_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Wartortle', 
    'Uncommon', 
    'Water', 
    '042/102', 
    'Good', 
    3.00, 
    12, 
    'https://images.pokemontcg.io/base1/42_hires.png'
);
-- common
CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Abra', 
    'Psychic', 
    'Common', 
    '043/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/43_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Bulbasaur', 
    'Grass', 
    'Common', 
    '044/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/44_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Caterpie', 
    'Grass', 
    'Common', 
    '045/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/45_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Charmander', 
    'Fire', 
    'Common', 
    '046/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/46_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Diglett', 
    'Fighting', 
    'Common', 
    '047/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/47_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Doduo', 
    'Colorless', 
    'Common', 
    '048/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/48_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Drowzee', 
    'Psychic', 
    'Common', 
    '049/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/49_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Gastly', 
    'Psychic', 
    'Common', 
    '050/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/50_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Koffing', 
    'Grass', 
    'Common', 
    '051/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/51_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Machop', 
    'Fighting', 
    'Common', 
    '052/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/52_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Magnemite', 
    'Electric', 
    'Common', 
    '053/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/53_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Metapod', 
    'Grass', 
    'Common', 
    '054/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/54_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Nidoran♂', 
    'Grass', 
    'Common', 
    '055/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/55_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Onix', 
    'Fighting', 
    'Common', 
    '056/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/56_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Pidgey', 
    'Colorless', 
    'Common', 
    '057/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/57_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Pikachu', 
    'Electric', 
    'Common', 
    '058/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/58_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Poliwag', 
    'Water', 
    'Common', 
    '059/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/59_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Ponyta', 
    'Fire', 
    'Common', 
    '060/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/60_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Rattata', 
    'Colorless', 
    'Common', 
    '061/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/61_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Sandshrew', 
    'Fighting', 
    'Common', 
    '062/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/62_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Squirtle', 
    'Water', 
    'Common', 
    '063/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/63_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Starmie', 
    'Water', 
    'Common', 
    '064/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/64_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Staryu', 
    'Water', 
    'Common', 
    '065/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/65_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Tangela', 
    'Grass', 
    'Common', 
    '066/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/66_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Voltorb', 
    'Electric', 
    'Common', 
    '067/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/67_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Vulpix', 
    'Fire', 
    'Common', 
    '068/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/68_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Weedle', 
    'Grass', 
    'Common', 
    '069/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/69_hires.png'
);
-- Trainer
CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Clefairy Doll', 
    'Trainer', 
    'Rare', 
    '070/102', 
    'Good', 
    1.20, 
    25, 
    'https://images.pokemontcg.io/base1/70_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Computer Search', 
    'Trainer', 
    'Rare', 
    '071/102', 
    'Good', 
    2.00, 
    15, 
    'https://images.pokemontcg.io/base1/71_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Devolution Spray', 
    'Trainer', 
    'Rare', 
    '072/102', 
    'Good', 
    1.50, 
    20, 
    'https://images.pokemontcg.io/base1/72_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Imposter Professor Oak', 
    'Trainer', 
    'Rare', 
    '073/102', 
    'Good', 
    2.00, 
    15, 
    'https://images.pokemontcg.io/base1/73_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Item Finder', 
    'Trainer', 
    'Rare', 
    '074/102', 
    'Good', 
    1.80, 
    20, 
    'https://images.pokemontcg.io/base1/74_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Lass', 
    'Trainer', 
    'Rare', 
    '075/102', 
    'Good', 
    1.50, 
    25, 
    'https://images.pokemontcg.io/base1/75_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Pokémon Breeder', 
    'Trainer', 
    'Rare', 
    '076/102', 
    'Good', 
    2.00, 
    15, 
    'https://images.pokemontcg.io/base1/76_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Pokémon Trader', 
    'Trainer', 
    'Rare', 
    '077/102', 
    'Good', 
    1.80, 
    20, 
    'https://images.pokemontcg.io/base1/77_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Scoop Up', 
    'Trainer', 
    'Rare', 
    '078/102', 
    'Good', 
    1.50, 
    25, 
    'https://images.pokemontcg.io/base1/78_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Super Energy Removal', 
    'Trainer', 
    'Rare', 
    '079/102', 
    'Good', 
    2.50, 
    10, 
    'https://images.pokemontcg.io/base1/79_hires.png'
);
-- Uncommon
CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Defender', 
    'Trainer', 
    'Uncommon', 
    '080/102', 
    'Good', 
    1.00, 
    30, 
    'https://images.pokemontcg.io/base1/80_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Energy Retrieval', 
    'Trainer', 
    'Uncommon', 
    '081/102', 
    'Good', 
    1.20, 
    25, 
    'https://images.pokemontcg.io/base1/81_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Full Heal', 
    'Trainer', 
    'Uncommon', 
    '082/102', 
    'Good', 
    1.00, 
    25, 
    'https://images.pokemontcg.io/base1/82_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Maintenance', 
    'Trainer', 
    'Uncommon', 
    '083/102', 
    'Good', 
    1.00, 
    25, 
    'https://images.pokemontcg.io/base1/83_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'PlusPower', 
    'Trainer', 
    'Uncommon', 
    '084/102', 
    'Good', 
    1.10, 
    25, 
    'https://images.pokemontcg.io/base1/84_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Pokémon Center', 
    'Trainer', 
    'Uncommon', 
    '085/102', 
    'Good', 
    1.30, 
    20, 
    'https://images.pokemontcg.io/base1/85_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Pokémon Flute', 
    'Trainer', 
    'Uncommon', 
    '086/102', 
    'Good', 
    1.20, 
    25, 
    'https://images.pokemontcg.io/base1/86_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Pokédex', 
    'Trainer', 
    'Uncommon', 
    '087/102', 
    'Good', 
    1.50, 
    20, 
    'https://images.pokemontcg.io/base1/87_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Professor Oak', 
    'Trainer', 
    'Uncommon', 
    '088/102', 
    'Good', 
    1.50, 
    20, 
    'https://images.pokemontcg.io/base1/88_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Revive', 
    'Trainer', 
    'Uncommon', 
    '089/102', 
    'Good', 
    1.00, 
    25, 
    'https://images.pokemontcg.io/base1/89_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Super Potion', 
    'Trainer', 
    'Uncommon', 
    '090/102', 
    'Good', 
    1.00, 
    30, 
    'https://images.pokemontcg.io/base1/90_hires.png'
);
-- Common
CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Bill', 
    'Trainer', 
    'Common', 
    '091/102', 
    'Good', 
    0.80, 
    35, 
    'https://images.pokemontcg.io/base1/91_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Energy Removal', 
    'Trainer', 
    'Common', 
    '092/102', 
    'Good', 
    0.90, 
    30, 
    'https://images.pokemontcg.io/base1/92_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Gust of Wind', 
    'Trainer', 
    'Common', 
    '093/102', 
    'Good', 
    1.00, 
    25, 
    'https://images.pokemontcg.io/base1/93_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Potion', 
    'Trainer', 
    'Common', 
    '094/102', 
    'Good', 
    0.80, 
    35, 
    'https://images.pokemontcg.io/base1/94_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Switch', 
    'Trainer', 
    'Common', 
    '095/102', 
    'Good', 
    0.90, 
    30, 
    'https://images.pokemontcg.io/base1/95_hires.png'
);
-- Energy
CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Double Colorless Energy', 
    'Energy', 
    'Uncommon', 
    '096/102', 
    'Good', 
    1.50, 
    25, 
    'https://images.pokemontcg.io/base1/96_hires.png'
);
CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Fighting Energy', 
    'Energy', 
    'Common', 
    '097/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/97_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Fire Energy', 
    'Energy', 
    'Common', 
    '098/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/98_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Grass Energy', 
    'Energy', 
    'Common', 
    '099/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/99_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Lightning Energy', 
    'Energy', 
    'Common', 
    '100/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/100_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Psychic Energy', 
    'Energy', 
    'Common', 
    '101/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/101_hires.png'
);

CALL AddCardToInventory(
    'Base Set', 
    'Base Set', 
    'Water Energy', 
    'Energy', 
    'Common', 
    '102/102', 
    'Good', 
    0.50, 
    40, 
    'https://images.pokemontcg.io/base1/102_hires.png'
);
